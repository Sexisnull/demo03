...
(function($, undefined) {
	...
	$.validity = {
		...
		patterns : {
			...
			zipcode : /^\d{6}$/, // 邮编
			mobile :  /^(13|15|18|17)[0-9]{9}$/, // 手机允许17打头的
		  ...
		  //username3 : /^(?!_)(?!.*?_$)[a-zA-Z0-9_]+$/, //用来注册外网用户，包括 个人和法人。但比username1严格的是：不能有汉字。且必须要以字母开头。李德隆于20160303
			username3 : /^(?!_)(?!.*?_$)[a-zA-Z][a-zA-Z0-9_]+$/, //用来注册外网用户，包括 个人和法人。但比username1严格的是：不能有汉字。且必须要以字母开头。李德隆于20160520

			letter : /^[A-Za-z]+$/, // 字母
			letter_u : /^[A-Z]+$/, // 大写字母
			letter_l : /^[a-z]+$/, // 小写字母
			codeid : /^[a-zA-Z0-9]+$/, // 机构标识。大小写字母及数字
			idcard : /^[1-9]([0-9]{14}|[0-9]{17})$/, // 身份证   其实此正则也并未用到。用的是更复杂的脚本判断。
