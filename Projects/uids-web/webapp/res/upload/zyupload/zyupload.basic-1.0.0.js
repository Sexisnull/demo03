...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B（允许上传文件最大'+para.fileSize/(1024*1024)+'M）。</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
			...
			this.funSetStatusInfo = function(files){
				...
				var max_size = para.fileSize/(1024*1024);
				// 设置内容
				$("#status_info").html("选中"+num+"张文件，共"+size+"（允许上传文件最大"+max_size+"M）。");
			};
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
//				var fileImgSrc = "/aais/res/upload/zyupload/skins/images/fileType/";
//				if(type == "rar"){
...
//				}
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '			<img id="uploadImage_'+file.index+'" class="upload_file '+type+'" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
					html += '		</div>';
					html += '	</a>';
					html += '	<p id="uploadProgress_'+file.index+'" class="file_progress"></p>';
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">上传失败，请重试</p>';
