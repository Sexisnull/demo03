...
(function($, undefined) {
	...
	$.validity = {
		...
		patterns : {
			...
			decmal : /^([+-]?)\d*\.\d+$/, // 娴偣鏁�
			decmal1 : /^[1-9]\d*.\d*|0.\d*[1-9]\d*$/,// 姝ｆ诞鐐规暟
			decmal2 : /^-([1-9]\d*.\d*|0.\d*[1-9]\d*)$/,// 璐熸诞鐐规暟
			decmal3 : /^-?([1-9]\d*.\d*|0.\d*[1-9]\d*|0?.0+|0)$/,// 娴偣鏁�
			decmal4 : /^[1-9]\d*.\d*|0.\d*[1-9]\d*|0?.0+|0$/,// 闈炶礋娴偣鏁帮紙姝ｆ诞鐐规暟 + 0锛�
			decmal5 : /^(-([1-9]\d*.\d*|0.\d*[1-9]\d*))|0?.0+|0$/,// 闈炴娴偣鏁帮紙璐熸诞鐐规暟 + 0锛�

			email : /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/, // 閭欢
			color : /^[a-fA-F0-9]{6}$/, // 棰滆壊
			chinese : /^[\u4E00-\u9FA5\uF900-\uFA2D]+$/, // 浠呬腑鏂�
			ascii : /^[\x00-\xFF]+$/, // 浠匒CSII瀛楃
			zipcode : /^\d{6}$/, // 閭紪
			mobile :  /^(13|15|18|17)[0-9]{9}$/, // 鎵嬫満鍏佽17鎵撳ご鐨�
			ip4 : /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5]).(\d{1,2}|1\d\d|2[0-4]\d|25[0-5]).(d{1,2}|1\d\d|2[0-4]\d|25[0-5]).(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/, // ip鍦板潃
			picture : /(.*)\.(jpg|bmp|gif|ico|pcx|jpeg|tif|png|raw|tga)$/, // 鍥剧墖
			rar : /(.*)\.(rar|zip|7zip|tgz)$/, // 鍘嬬缉鏂囦欢
			zip : /(.*)\.(zip)$/,
			qq : /^[1-9]*[1-9][0-9]*$/, // QQ鍙风爜
			tel : /(\d{3}-|\d{4}-)?(\d{8}|\d{7})/, // 鍥藉唴鐢佃瘽
			username : /^\w+$/, // 鐢ㄦ潵鐢ㄦ埛娉ㄥ唽銆傚尮閰嶇敱鏁板瓧銆�26涓嫳鏂囧瓧姣嶆垨鑰呬笅鍒掔嚎缁勬垚鐨勫瓧绗︿覆
			username1 : /^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5]+$/,// 鐢ㄦ潵鐢ㄦ埛娉ㄥ唽銆傚尮閰嶅瓧姣嶃€佹暟瀛椼€佷笅鍒掔嚎銆佷腑鏂囩粍鎴愶紝涓嶈兘浠ヤ笅鍒掔嚎寮€澶村拰缁撳熬
			username2 : /^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5,]+$/,// 鐢ㄦ潵鐢ㄦ埛娉ㄥ唽銆傚尮閰嶅瓧姣嶃€佹暟瀛椼€佷笅鍒掔嚎銆佷腑鏂囥€佽嫳鏂囬€楀彿缁勬垚锛屼笉鑳戒互涓嬪垝绾垮紑澶村拰缁撳熬
		  //username3 : /^(?!_)(?!.*?_$)[a-zA-Z0-9_]+$/, //鐢ㄦ潵娉ㄥ唽澶栫綉鐢ㄦ埛锛屽寘鎷� 涓汉鍜屾硶浜恒€備絾姣攗sername1涓ユ牸鐨勬槸锛氫笉鑳芥湁姹夊瓧銆備笖蹇呴』瑕佷互瀛楁瘝寮€澶淬€傛潕寰烽殕浜�20160303
			username3 : /^(?!_)(?!.*?_$)[a-zA-Z][a-zA-Z0-9_]+$/, //鐢ㄦ潵娉ㄥ唽澶栫綉鐢ㄦ埛锛屽寘鎷� 涓汉鍜屾硶浜恒€備絾姣攗sername1涓ユ牸鐨勬槸锛氫笉鑳芥湁姹夊瓧銆備笖蹇呴』瑕佷互瀛楁瘝寮€澶淬€傛潕寰烽殕浜�20160520

			letter : /^[A-Za-z]+$/, // 瀛楁瘝
			letter_u : /^[A-Z]+$/, // 澶у啓瀛楁瘝
			letter_l : /^[a-z]+$/, // 灏忓啓瀛楁瘝
			codeid : /^[a-zA-Z0-9]+$/, // 鏈烘瀯鏍囪瘑銆傚ぇ灏忓啓瀛楁瘝鍙婃暟瀛�
			idcard : /^[1-9]([0-9]{14}|[0-9]{17})$/, // 韬唤璇�   鍏跺疄姝ゆ鍒欎篃骞舵湭鐢ㄥ埌銆傜敤鐨勬槸鏇村鏉傜殑鑴氭湰鍒ゆ柇銆�
