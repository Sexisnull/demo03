...
public class AuthLogService{
	...
	private static Logger logger = LoggerFactory.getLogger(AuthLogService.class);
  ...
  public boolean modifyJisAuthLog(JisAuthlog jisAuthLog)
  {
    ...
    return this.authLogDAO.updateAuthLog(jisAuthLog);
  }
  ...
  public boolean addJisAuthLog(JisAuthlog jisAuthLog)
  {
    ...
    int iid = ((Integer)this.authLogDAO.insert(jisAuthLog)).intValue();
    return iid > 0;
  }
  ...
  public boolean removeAuthLogByIds(String ids)
  {
    ...
    boolean isSuccess = this.authLogDAO.deleteByIds(idsLsit);
    if (!isSuccess) {
      logger.error("删除票据失败!");
    }
    return isSuccess;
