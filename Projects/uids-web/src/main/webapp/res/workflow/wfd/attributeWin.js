...
AttributeWin = function() {
	...
	var formInfo = new Ext.form.ComboBox({
		...
		var orgInfo = new Ext.form.ComboBox({
		store : new Ext.data.Store({
			proxy : new Ext.data.HttpProxy({
				url : Workflow.ORG_LIST_URL
			}),
			reader : new Ext.data.JsonReader({
				totalProperty : 'totalProperty',
				root : "orglist"
			...
			listeners : {
				load : function() {
					...
					if (this.getAt(0)) {
						var thisvalue = this.getAt(0).get('id');
						var index = this.find('id', orgInfo.getValue());
						if (index == -1) {
							orgInfo.setValue(thisvalue);
						}
					} else {
						orgInfo.setValue("");
					}
		...
		listeners : {
			select : function(leagueCombox, record, index) {
			//Ext.extend.setOrgId(record.get('id'));
			//__this.model.setOrgId(record.get('id'));
			//	alert("sss"+record.get('id'));
			//	var orgId=record.get('id');
			//	var form = document.forms['saveForm'];
			//    form.elements['orgId'].value = orgId;
			//    alert(form.elements['orgId'].value);
			}
		}
	});
	...
	var processAttr = new Ext.Panel({
		...
		formInfo,orgInfo, new Ext.form.TriggerField({
			fieldLabel : '督办人', // 主办人
			name : 'mainHandler',
			id : 'mainHandler',
			...
			onTriggerClick : function() {
				if (!this.disabled) {
					...
					var sFeatures = "dialogLeft:" + nLeft + "px;dialogTop:"
							+ nTop + "px;dialogHeight:" + nDialogHeight
							+ "px;dialogWidth:" + nDialogWidth
							+ "px;help:no;status:yes;scroll:no;";

					obj.userList = __this.mainHandler.id
							? __this.mainHandler.id
							: "";
					obj.userNames = __this.mainHandler.name
							? __this.mainHandler.name
							: "";
					obj.openModel = {
	...
	var selectRunner = function() {
		...
		obj.openModel = {  
			hiddenViewer : [],
			selectFlowVar : true, selectRecentContacts : false, selectUsedContact : false
		};
		...
		obj.orgNames = __this.runner.orgNames;
		...
		__this.actiongroup.items.items.each(function(item){
  		...
  		obj.openModel = {
			...
			_processSelect : true, _recentSelect : false, _commonSelect : false
		};
		window.showModalDialog(Workflow.RUNNER_URL, obj, sFeatures);
		if (obj.isSave)
	...
	for (var i = 0; i < actions.length; i++) {
		...
		var checkbox = new Ext.form.Checkbox({
					...
					// hidden : true,
					handler : function() {
						if (this.inputValue == Workflow.MSG_ACTION) {
							if (this.checked) {
	...
	var tabpanel = new Ext.TabPanel({
		...
		items : [processAttr, note, this.baseMsg, this.formCtrl, runner,
				timeSet, this.treadSet, condition]
	});
...
Ext.extend(AttributeWin, Ext.util.Observable, {
	setWorkflowModel : function(model) {
		...
		if(model.isGuide()){
			...
			var orgInfo = this.form.findField("org");
			if(orgInfo){
				orgInfo.store.proxy.conn.url = Workflow.ORG_LIST_URL + model.getType();
				orgInfo.store.reload();
			}
		}
	},
	show : function(model) {
	...
	dealRunner : function(obj, activity) {
		...
		this.runner.orgNames = '';

		if (obj.userList) {
			if(obj.userList.indexOf('_')!=-1){
				obj.userList = obj.userList.substring(obj.deptList.userList('_')+1 );
			}
		...
		if (obj.deptList) {
			...
			this.runner.orgNames = obj.orgNames;
		}
		if (obj.groupList) {
	...
	setRunnerDescValue : function(obj) {
		...
		obj.deptNames = obj.orgNames ? obj.orgNames : '';
