WFD = function(editState){
	...
	this.toolspanel.addButton(new Button(1,"../res/workflow/images/select.gif"),"选择");
	this.toolspanel.addButton(new Button(3,"../res/workflow/images/start16.png"),"开始");
	this.toolspanel.addButton(new Button(11,"../res/workflow/images/node16.png"),"普通");
	this.toolspanel.addButton(new Button(5,"../res/workflow/images/end16.png"),"结束");
	this.toolspanel.addButton(new Button(13,"../res/workflow/images/transition.gif"),"连接");
	this.toolspanel.addButton(new Button(15,"../res/workflow/images/bzzw.gif"),"标注");
...
//	        },
			             	...
			             	if(validateSave.actionPerformed()){
			             		...
			             		if (domXml) {
			             			...
			             			form.elements['workflowXml'].value = domXml;
			 	       				...
			 	       				new Ajax.Request("workflowTemplate!saveForAjax.do", { 
			 	       					asynchronous: false, 
			 	       					method: 'post', 
			 	       					...
			 	       					onComplete: function(req){
			        	                    	...
			        	                    	if(result!=null && result.length == 32){
			     	                    		...
			     	                    		Wrapper.setChanged(false);
			        	                    		...
			        	                    		window.close();
			        	                    		window.location.href="workflowTemplate!list.do";
		...
		tb.addButton({
			            ...
			            handler : function(){
			            	//添加关闭方法，页面关闭
			            	var iframeDiv=document.getElementById("viewcontainer");
...
//			                window.opener.select_unmask();
	...
	addViewToolBar : function(Wrapper){
		...
		tb.addButton({
			            ...
			            iconCls:'exit',
			            handler : function(){
							var _id=document.getElementById("windowId").value;
							  // alert(_id);
            	...
            	if(validateSave.actionPerformed()){
            		...
            		if (domXml) {
            			...
            			form.elements['workflowXml'].value = domXml;
	       				form.elements['templateId'].value = Wrapper.getModel().getId();
	       				form.elements['wtType'].value = Wrapper.getModel().getWtType();
	       				new Ajax.Request("workflowTemplateAais!saveForAjax.do", { 
	       					asynchronous: false, 
	       					method: 'post', 
	       					parameters: Form.serialize(form), 
	       					onComplete: function(req){
...
//		this.attributeWin.show(null,this.model );
		...
		if(this.wflogGrid){//加载流程日志
			this.wflogGrid.store.proxy.conn.url = Workflow.WORK_LOG + model.getId();  
	...
	addViewerListeners : function(Wrapper){
		...
		var rightMenu = new Ext.menu.Menu ([
				{xtype:"button",text:"属性",icon:"../res/workflow/images/attribute.gif",pressed:true,handler:function(){
...
//				{xtype:"button",text:"添加拐点",icon:"nodes/start.gif",pressed:true,handler:function(){}},
                {xtype:"button",text:"删除",icon:"../res/workflow/images/delete.gif",pressed:true,disabled:false,handler:function(){if(Wrapper.getModel().hasSelect()){if(confirm('确定删除所选吗?')){Wrapper.getModel().deleteSelect();}}}},
                {xtype:"button",text:"全选",icon:"../res/workflow/images/start.gif",pressed:true,handler:function(){Wrapper.getModel().selectAll();}}
//                {xtype:"button",text:"xml",icon:"../res/workflow/images/xml.gif",pressed:true,
//                	handler:function(){
//                		Ext.MessageBox.show({
//				           title: 'xml',
//				           msg: '流程设计输出xml',
...
//					Wrapper.getModel().addActivity(nodeModel);
	...
	initContainer : function(){
		...
		this.contentPanel = new Ext.Panel({
			...
			layout: 'absolute',
			margins: '2 5 5 0',
			bodyStyle:'background-image:url("../res/workflow/images/workbg.gif");overflow: scroll;',
			border: false
		});
...
//			            defaultText: '欢迎使用万维流程定制系统',
				        ...
				        items: [
				        	{
					            text: '关于',
					            handler : function(){
					            	Ext.MessageBox.alert("","综合办公网络版 流程设计器V3.0");
					            }
	    ...
	    var store = new Ext.data.Store({
	    	url: 'workflowTemplate!workflowLog.do',
...
//	    [{"activityName":"开始","dealTime":"2009-08-10 10:54:00","fromInfo":"","logContent":"提交办理","taskAssignTime":"2009-08-10 10:53:36","userName":"张京"}]
	    ...
	    this.wflogGrid = new Ext.grid.GridPanel({
	        ...
	        columns: [
	            ...
	            {header: "办理人",width:120,dataIndex: 'userName', sortable: true},
	            {header: "来源信息",width:320,dataIndex: 'fromInfo', sortable: true},
	            {header: "操作",width:120,dataIndex:'logContent', sortable: true},
	            {header: "任务到达时间",width:150,dataIndex: 'taskAssignTime', sortable: true},
	            {header: "办理时间",width:150,dataIndex: 'dealTime', sortable: true},
	            {header: "办理时长",width:150,dataIndex: 'dealTimeDeff', sortable: true}
	        ],
