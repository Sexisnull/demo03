...
Date.defMonthNames = [ "一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月",
		"十月", "十一月", "十二月" ];
Date.defAbbrMonthNames = [ "一", "二", "三", "四", "五", "六", "七", "八", "九", "十",
		"十一", "十二" ];
...
(function(V) {
	var E = V.fn.domManip, S = "_tmplitem", F = /^[^<]*(<[\w\W]+>)[^>]*$|\{\{\! /, U = {}, Q = {}, R, G = {
		key : 0,
		data : {}
	}, N = 0, T = 0, K = [];
	function P(b, i, a, f) {
	...
	V.each( {
		...
		V.fn[a] = function(s) {
			var r = [], p = V(s), e, q, c, d, f = this.length === 1
					&& this[0].parentNode;
			R = U || {};
	...
	V.extend( {
		...
		tmplItem : function(a) {
			...
			while (a && a.nodeType === 1 && !(d = V.data(a, "tmplItem"))
					&& (a = a.parentNode)) {
			}
			return d || G
		},
		template : function(d, a) {
			...
			return d ? typeof d !== "string" ? V.template(null, d)
					: V.template[d] || V.template(null, F.test(d) ? d : V(d))
					: null
		},
		encode : function(b) {
	...
	function M(i, d, h) {
		var a, j = h ? V.map(h, function(b) {
			return typeof b === "string" ? i.key ? b.replace(
					/(<\w+)(?=[\s>])(?![^>]*_tmplitem)([^>]*)/g, "$1 " + S
							+ '="' + i.key + '" $2') : b : M(b, i, b._ctnt)
		}) : i;
		if (d) {
			return j
		}
		j = j.join("");
		j.replace(/^\s*([^<\s][^<]*)?(<[\w\W]+>)([^>]*[^>\s])?\s*$/, function(
				b, l, g, k) {
			a = V(g).get();
			J(a);
			if (l) {
	...
	function I(d, a) {
		d._wrap = M(d, true, V.isArray(a) ? a : [ F.test(a) ? a : V(a).html() ])
				.join("")
	}
	function O(b) {
	...
	function J(b) {
		...
		function d(s) {
			...
			if (l = s.getAttribute(S)) {
				while (t.parentNode && (t = t.parentNode).nodeType === 1
						&& !(v = t.getAttribute(S))) {
				}
				if (v !== l) {
					t = t.parentNode ? t.nodeType === 11 ? 0 : t
							.getAttribute(S) || 0 : 0;
					if (!(u = U[l])) {
						u = Q[l];
						u = P(u, U[t] || Q[t]);
	...
	function A(e, f) {
		...
		return V.map(V(V.isArray(a) ? a.join("") : a).filter(e || "*"),
				function(b) {
					return f ? b.innerText || b.textContent : b.outerHTML
							|| D(b)
				})
	}
	function C() {
...
(function(b) {
	...
	b.download = function(e, g, h) {
		if (e && g) {
			var c = "", d, f = b("<form />").attr("action", e).attr("method",
					(h || "post"));
	...
	b.formatDateTime = function(c) {
		if (typeof c === "string") {
			...
			var c = new Date(c);
			return b.format("{0}-{1}-{2} {3}:{4}:{5}", c.getFullYear(), b
					.formatHH(c.getMonth() + 1), b.formatHH(c.getDate()), b
					.formatHH(c.getHours()), b.formatHH(c.getMinutes()), b
					.formatHH(c.getSeconds()))
		}
	};
	b.formatDate = function(c) {
		if (typeof c === "string") {
			...
			var c = new Date(c);
			return b.format("{0}-{1}-{2}", c.getFullYear(), b.formatHH(c
					.getMonth() + 1), b.formatHH(c.getDate()))
		}
	};
	b.formatTime = function(c) {
		if (typeof c === "string") {
			...
			var c = new Date(c);
			return b.format("{0}:{1}", b.formatHH(c.getHours()), b.formatHH(c
					.getMinutes()))
		}
	};
	...
	b.getDate = function(g) {
		var f = g.find(".yyyy").val(), e = g.find(".mm").val(), d = g.find(
				".dd").val();
		if (f || e || d) {
	...
	b.getTime = function(g) {
		var e = g.find(".hh").val(), d = g.find(".mi").val(), f = g.find(".se")
				.val();
		if (e || d || f) {
	...
	b.getName = function(g) {
		var f = g.find(".n1").val(), e = g.find(".n2").val(), d = g.find(".n3")
				.val();
		if (f || e || d) {
	...
	b.alert = function(f, g) {
		...
		if (d.length == 0) {
			...
			e = b('<div id="lightBoxAlert" class="lightbox alert hide"><div id="lbContentAlert" class="lbcontent alert clearfix">'
					+ c + "</div></div>");
			b("body").append(d).append(e)
		} else {
			e.addClass("alert").find("#lbContentAlert").addClass("alert")
					.empty().append(c);
			d.addClass("alert")
		}
	...
	b.setCookie = function(d, e, c) {
		...
		document.cookie = d + "=" + escape(e)
				+ ((c == null) ? "" : ";expires=" + f.toGMTString())
	};
	...
	b.isMobile = function(c) {
		...
		return /^((13[0-9])|(14[0-9])|(15[0-9])|(18[0-9])|(17[0-9]))\d{8}$/
				.test(c)
	};
	b.showStatus = function(c) {
		...
		if (d.length === 0) {
			d = b('<div id="status" class="status"><div id="y" class="y" style="top:0xp;left:0px"><div id="statusText" class="statusText">'
					+ c + "</div></div></div>");
			b("body").append(d)
		}
		b("#statusText").text(c);
	...
	b.initDate = function(c, d) {
		...
		if (b.fn.datepicker) {
			c.find("input.datepincker").datepicker( {
				showOn : "button",
				buttonImage : "/rs/css/images/calendar.gif",
	...
	b.fn.outerHTML = function(c) {
		return (c) ? this.before(c).remove() : b("").append(this.eq(0).clone())
				.html()
	};
	...
	b.fn.getValues = function() {
		...
		o = {};
		b(
				":text,select,input:checked,textarea,input[type=hidden],input[type=number],input[type=date],input[type=tel],input[type=email],input[type=url],input[type=time],:password",
				this).each(
				function(f, h) {
					var j = b(h), c = j.attr("name"), g = j.attr("type"), e = b
							.trim(j.val())
							|| "";
					if (!c) {
						return true
		...
		b(":checkbox", this).each(
				function(e, f) {
					var h = b(f), c = h.attr("name"), g = b(":checkbox[name='"
							+ c + "']", d).length;
					if (!c) {
						return true
					}
				...
				if (g.is(":checkbox", h)) {
					if (b.isArray(i)) {
						b.each(i, function(k, m) {
							m = b.formatHTMLValue(m);
							g.filter("[value='" + m + "']").prop("checked",
									true)
						})
					} else {
						i = b.formatHTMLValue(i);
						g.filter("[value='" + i + "']").prop("checked", true)
					}
				} else {
					if (g.is(":radio", h)) {
						i = b.formatHTMLValue(i);
	...
	b.lightBox = function(k) {
		var e = {
		...
		}, c = b.extend(e, k), g = b(c.overlay), f = b(c.lightBox), h = b(c.content), d = function() {
			if (c.url.indexOf("#") == 0) {
				var l = h.find(">div,>form");
				b("#divtmp").append(l)
			}
			f.hide();
			g.hide();
			...
			b(c.overlay).unbind().click(function() {
				d();
				b("#lightBox").css( {
					width : "",
					marginLeft : ""
		...
		if (c.url) {
			if (b.data(document, c.url)) {
				...
				if (c.url.indexOf("#") == 0) {
					h.html("").append(b(c.url).show());
					i();
					c.loaded()
				} else {
					b.get(c.url, function(l) {
						h.html(l);
						b.data(document, c.url, l);
						i();
						c.loaded()
					})
				}
		...
		if (b.browser.msie && b.browser.version === "6.0") {
			...
			if (b("#sIframe").length === 0) {
				b("body").append(
						"<iframe id='sIframe' class='sIframe hide'></iframe>")
			}
			var j = b("#sIframe");
			j.width(g.width());
			j.height(g.height());
...
Date.prototype.addDate = function(a, c) {
	...
	case "m":
		return new Date(b.getFullYear(), b.getMonth() + c, b.getDate(), b
				.getHours(), b.getMinutes(), b.getSeconds());
	case "y":
		return new Date(b.getFullYear() + c, b.getMonth(), b.getDate(), b
				.getHours(), b.getMinutes(), b.getSeconds())
	}
};
Date.prototype.toString = function() {
	return $.format("{0}-{1}-{2}", this.getFullYear(), $.formatHH(this
			.getMonth() + 1), $.formatHH(this.getDate()))
};
