...
var setting = {
	...
	var zNodes =[];
		...
		function beforeRemove(treeId, treeNode) {
			...
			$.ajax({
				 type: "POST",
				 url: "/center/sys/department/checkUser",
		 ...
		 function zTreeOnRemove(event,treeId, treeNode) {
	         ...
	         //ajax调用后台进行数据库删除
				 if (treeNode.id != undefined) {
					 $.ajax({
						 type: "POST",
						 url: "/center/sys/department/delete",
						 data: "key=" + treeNode.tld,
						 dataType: "text",
						 success: function (data) {
						 if (data == "success") {
	         ...
	         if (newName.length == 0) {
	          ...
	          if (newName != treeNode.name) {
	             if (1==1) {
	                 $.ajax({
	                 type: "POST",
	                 url: "/center/sys/department/updateName",
	                 ...
	                 success: function (data) {
	                     ...
	                     $.ajax({
	  					   type: "POST",
	  					   url: "/center/sys/department/getRegion",
	  					   data: "key="+treeNode.tld,
	  				       dataType: "json",
	  					   success: function(data){
	  						 var json=data;
	  					     ...
	  					     $("#groupallname").attr("value",json.groupallname);
	  					     $("#nodetype").attr("value", json.nodetype);
	  					     $("#orgcode").attr("value", json.orgcode);
	  					     $("#orgtype").attr("value", json.orgtype);
	  					     $("#areatype").attr("value", json.areatype);
	  					     $("#areacode").attr("value", json.areacode);
	  					   $("#iscombine").attr("value", json.iscombine);
	  					     $("#suffix").attr("value",json.suffix);
	  					     $("#spec").attr("value", json.spec);
	  					     $("#pinyin").attr("value", json.pinyin);
		...
		function addHoverDom(treeId, treeNode) {
			...
			if (btn) btn.bind("click", function(){
				...
				$.ajax({
					   type: "POST",
					   url: "/center/sys/department/save",
				...
				if(key!=undefined){
					$.ajax({
					   ...
					   success: function(data){
					     ...
					     $("#groupallname").attr("value",jsonStr.groupallname);
  					     $("#nodetype").attr("value", jsonStr.nodetype);
  					     $("#orgcode").attr("value", jsonStr.orgcode);
  					     $("#orgtype").attr("value", jsonStr.orgtype);
  					     $("#areatype").attr("value", jsonStr.areatype);
  					     $("#areacode").attr("value", jsonStr.areacode);
  					     $("#iscombine").attr("value", jsonStr.iscombine);
  					     $("#suffix").attr("value",jsonStr.suffix);
  					     $("#spec").attr("value", jsonStr.spec);
  					     $("#pinyin").attr("value", jsonStr.pinyin);
