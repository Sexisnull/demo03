...
public class ImmediateSyncThread extends Thread {
	...
	public void run() {
		try {
			String transcationId = syncMap.get("transcation_id").toString();//本条同步记录交互ID
			logger.info("本次同步开始,本条记录transcationId：" + transcationId);
			System.out.println("本次同步开始,本条记录transcationId：" + transcationId);
			...
			System.out.println("实时同步数据表删除本条记录IID=" + iid);
			...
			if (Constants.TRANS_HTTP == transtype) {
				...
				Map<String, Object> sendMap = sysViewDetailDao.findDetailById(transcationId);
			...
			logger.info("错误记录transcationId为:" + syncMap.get("transcation_id") + " e.getMessage():" + e.getMessage());
			e.printStackTrace();
		}
